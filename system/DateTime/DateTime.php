...
class _DateTime extends \DateTime
{
	 ...
	 * @return	string
	 */
	public function localeTimeFormat( $seconds=FALSE, $minutes=TRUE, $memberOrLanguage=NULL )
	{
	...
	{
		return static::determineLanguage( $memberOrLanguage )->convertString( strftime( $this->localeTimeFormat( $seconds, $minutes, $memberOrLanguage ), $this->getTimestamp() + $this->getTimezone()->getOffset( $this ) ) );
	}
	...
	public function relative( $format=0, $memberOrLanguage=NULL )
	{
		...
		if( $format == static::RELATIVE_FORMAT_SHORT AND !$difference->invert and $now->format( 'Y', $language ) != $this->format( 'Y', $language ) )
		{
			if( $difference->y )
			{
				return $language->addToStack( 'f_years_short', FALSE, array( 'sprintf' => array( $difference->y ) ) );
			}
			else
			{
		...
		if ( !$difference->invert and $now->format( 'Y', $language ) == $this->format( 'Y', $language ) )
		{
			...
			elseif ( $difference->d )
			{
				...
				{
					return $language->addToStack( "_date_this_week{$capitalKey}", FALSE, array( 'sprintf' => array( $this->strFormat( '%A', $language ), $this->localeTime( FALSE, TRUE, $language ) ) ) );
				}
			}
			...
			elseif ( $difference->h )
			{
				...
				if ( $format == static::RELATIVE_FORMAT_SHORT )
				{
					return $language->addToStack( 'f_hours_short', FALSE, array( 'sprintf' => array( $difference->h ) ) );
				}
			...
			elseif ( $difference->i )
			{
				...
				if ( $format == static::RELATIVE_FORMAT_SHORT )
				{
					return $language->addToStack( 'f_minutes_short', FALSE, array( 'sprintf' => array( $difference->i ) ) );
				}
			...
			else
			{
				if ( $format == static::RELATIVE_FORMAT_SHORT )
				{
					return $language->addToStack( 'f_minutes_short', FALSE, array( 'sprintf' => array( 1 ) ) );
				}
				else
				{
