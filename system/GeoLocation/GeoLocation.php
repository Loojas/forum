...
class _GeoLocation
{
	...
	{
		if ( \IPS\Settings::i()->google_maps_api_key )
		{
	...
	{
		if ( \IPS\Settings::i()->google_maps_api_key AND $this->toString() )
		{
			try
			{
				$data = \IPS\Http\Url::external( "https://maps.googleapis.com/maps/api/geocode/json" )->setQueryString( array(
	...
	public function toString( $separator=', ', $name=NULL )
	{
		...
		if ( $this->country and $this->country !== static::buildFromJson( \IPS\Settings::i()->site_address )->country )
		{
			...
			{
				$output[] = htmlspecialchars( $this->country, \IPS\HTMLENTITIES, 'UTF-8', FALSE );
			}
		}
