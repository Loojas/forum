		...
		{			
	        try
	        {
	            ...
	            if( \IPS\Request::i()->embedComment )
	            {
		            ...
		            $content = $commentClass::loadAndCheckPerms( \IPS\Request::i()->embedComment );
		            $title = $content->item()->mapped('title');
				}
				elseif( \IPS\Request::i()->embedReview )
	            {
		            $reviewClass = $class::$reviewClass;
		            $content = $reviewClass::loadAndCheckPerms( \IPS\Request::i()->embedReview );
		            $title = $content->item()->mapped('title');
				}
				else
				{
					...
					$content = $class::loadAndCheckPerms( \IPS\Request::i()->id );
