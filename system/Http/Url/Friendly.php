...
class _Friendly extends \IPS\Http\Url\Internal
{
	...
	protected static function buildFurlConfiguation()
	{
		...
		foreach ( $applications as $app )
		{
			...
			if( file_exists( \IPS\ROOT_PATH . "/applications/{$app->directory}/data/furl.json" ) )
			{
				...
				{
					$definitions[ $k ] = static::buildFurlDefinition( $page['friendly'], $page['real'], $topLevel, $app->default, isset( $page['alias'] ) ? $page['alias'] : NULL, FALSE, isset( $page['verify'] ) ? $page['verify'] : NULL );
				}
	 ...
	 * @return	array
	 */
	public static function buildFurlDefinition( $friendly, $real, $appTopLevel = NULL, $appIsDefault = FALSE, $alias = NULL, $custom = FALSE, $verify = NULL )
	{
		/* Init */
		$return = array(
			'friendly'	=> $friendly,
