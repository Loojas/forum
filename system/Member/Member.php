...
class _Member extends \IPS\Patterns\ActiveRecord
{
	...
	public function location()
	{
		...
		if( $this->sessionData === NULL )
		{
			try
			{
				$this->sessionData = \IPS\Db::i()->select( '*', 'core_sessions', array( 'member_id=?', $this->member_id ), 'running_time DESC' )->first();
			}
	...
	public static function photoUrl( $memberData, $thumb=TRUE, $email=FALSE, $useDefaultPhoto=TRUE )
	{
		...
		if ( isset( $memberData['member_id'] ) and $memberData['member_id'] )
		{
			...
			elseif ( \IPS\Settings::i()->allow_gravatars and ( $memberData['pp_photo_type'] === 'gravatar' or ( !$memberData['pp_photo_type'] and !$gravatarDisabled ) ) )
			{
				...
				$defaultUrl = \IPS\IN_DEV ? '' : \IPS\Theme::i()->resource( 'default_photo.png', 'core', 'global' )->setScheme( \IPS\Request::i()->isSecure()  ? 'https' : 'http' );
	 ...
	 * @apiresponse	string										timezone				Member timezone
	 ...
	 * @apiresponse	datetime									lastActivity			Last activity date on the site
	 * @apiresponse	datetime									lastVisit				Last distinct visit date on the site
	 * @apiresponse	datetime									lastPost				Latest content submission date
	...
	public function apiOutput()
	{
		...
		return array(
			...
			'posts'					=> $this->member_posts,
			'lastActivity'			=> \IPS\DateTime::ts( $this->last_activity )->rfc3339(),
			'lastVisit'				=> \IPS\DateTime::ts( $this->last_visit )->rfc3339(),
			'lastPost'				=> \IPS\DateTime::ts( $this->member_last_post )->rfc3339(),
			'profileViews'			=> $this->members_profile_views,
			'birthday'				=> $this->bday_month ? ( $this->bday_month . '/' . $this->bday_day . ( $this->bday_year ? '/' . $this->bday_year : '' ) ) : NULL,
			'customFields'			=> $fields
		);
	}
	...
	public function checkLoginKey()
	{
		...
		if ( !$this->member_login_key or $keyExpiry->diff( \IPS\DateTime::create() )->days > 90 )
		{
			...
			$this->member_login_key_expire 	= \IPS\DateTime::create()->add( new \DateInterval( 'P3M' ) )->getTimestamp();
	...
	public function checkGroupPromotion()
	{
		if ( $this->group['g_promotion'] != '-1&-1' )
		{
			...
			{
				/* Are we checking for post based auto incrementation? 0 is post based, 1 is date based, 2 is reputation based so...  */
				if ( $this->group['g_promotion_type'] == 1 )
				{
				...
				else if ( $this->group['g_promotion_type'] == 2 )
				{
					if ( $this->pp_reputation_points >= $gUnit )
					{
						$this->member_group_id = $gid;
					}
				}
	...
	public function hasNameChanges()
	{
		try
		{
			return \IPS\Db::i()->select( 'count(*)', 'core_dnames_change', array( 'dname_member_id=?', $this->member_id ) )->first();
		}
		catch ( \UnderflowException $e )
		{
			return FALSE;
	...
	/**
	 * Linked profile sync services
	 *
	 * @return array
	 */
	public function connectedProfileSyncServices()
	{
		$services = array();

		foreach( \IPS\core\ProfileSync\ProfileSyncAbstract::services() as $service )
		{
			$obj = new $service( $this );
			if ( $obj->connected() )
			{
				$services[] = $obj::$icon;
			}
