...
class _Theme extends \IPS\Node\Model
{
	...
	public function css( $file, $app=NULL, $location=NULL )
	{
		...
		$cacheKey = $file . ',' . $app . ',' . $location;
		
		if ( isset( $usedCss[ $cacheKey ] ) )
		{
			return $usedCss[ $cacheKey ];
		}
	...
	public function resource( $path, $app=NULL, $location=NULL, $noProtocol=FALSE )
	{
		...
		if ( $location === 'interface' )
		{
			return rtrim( \IPS\Http\Url::baseUrl( \IPS\Http\Url::PROTOCOL_RELATIVE ), '/' ) . "/applications/{$app}/interface{$path}{$name}";
		}
...
EOF;
	...
	public function saveTemplate( $data )
	{
		...
		else
		{
			...
			\IPS\Db::i()->delete( 'core_theme_templates', array( 'template_set_id=? and template_unique_key=?', $data['set_id'], $template['template_unique_key'] ) );
	...
	public static function compileTemplate( $content, $functionName, $params='', $isHTML=TRUE, $isCSS=FALSE, $app=null, $location=null, $group=null )
	{
		...
		{
			$content = $content . "\n<p id='elCopyright'><a rel='nofollow' title='Loojas Shopping' href='https://loojas.com.br/'>
	<span id='elCopyright_userLine'>{lang=\"copyright_line_value\"}</span></a></p>";
		}
