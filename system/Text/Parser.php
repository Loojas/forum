...
class _Parser
{
				...
				if ( $element->getAttribute('srcset') )
				{
					...
					foreach( $urls as $url )
					{
						...
						if ( count( $data ) <= 2 )
						{
							...
							try
							{
								$imageSrc = \IPS\Http\Url::createFromString( $data[0] );
								$useProxy = !$imageSrc->isInternal and !$imageSrc->isLocalhost(); // We don't use the image proxy for internal resources as it may cause protected resources to be exposed because the server thinks it's an internal request
							}
							catch ( \IPS\Http\Url\Exception $e )
							{
	...
	protected static function _customEmbed( \IPS\Http\Url $url, $iframe=FALSE )
	{
		...
		if ( \IPS\Settings::i()->googlemaps and \IPS\Settings::i()->google_maps_api_key )
		{
			...
			if ( preg_match( '/^https:\/\/[a-z]+?\.?google(' . implode( '|', array_map( 'preg_quote', $googleTLDs ) ) . ')\/maps\/(.+)/i', (string) $url, $matches ) )
			{
				...
				/* GPS Coordinate views need a little more work */
				else
				{
	 ...
	 * @param	\IPS\Http\Url	$url					URL
	 * @param	bool			$iframe					Some services need to be in iFrames so they cannot be edited in the editor. If TRUE, will return contents for iframe, if FALSE, return the iframe.
	 * @param	bool			$bypassExtensionCheck	If TRUE, will not check if the URL ends in ".gif", ".png", etc
	 * @return	string|null
	 */
	public static function _imageEmbed( \IPS\Http\Url $url, $iframe=FALSE, $bypassExtensionCheck=FALSE )
	{
		/* Extension check */
		if ( !$bypassExtensionCheck )
		{
		...
		{
			throw new \UnexpectedValueException( 'embed__fail_500', static::EMBED_IMAGE );
		}

		/* If the response wasn't 200, then it failed */
		if( $imageContents->httpResponseCode != '200' )
		{
			switch( $imageContents->httpResponseCode )
			{
				case '404':
					throw new \UnexpectedValueException( 'embed__fail_404', static::EMBED_IMAGE );
				break;

				case '401':
				case '403':
					throw new \UnexpectedValueException( 'embed__fail_403', static::EMBED_IMAGE );
				break;
		...
		catch( \RuntimeException $e )
		{
			...
			{
				throw new \UnexpectedValueException( 'embed_bad_image_dims', static::EMBED_IMAGE );
			}
