...
class _ItemController extends \IPS\Api\Controller
{
	...
	protected function _list( $where = array(), $containerParam = 'categories' )
	{
		...
		return new \IPS\Api\PaginatedResponse(
			200,
			\IPS\Db::i()->select( '*', $class::$databaseTable, $where, "{$sortBy} {$sortDir}" ),
	...
	protected function _createOrUpdate( \IPS\Content\Item $item )
	{
		...
		/* Update first comment if required */
		$field = $item::$databaseColumnMap['first_comment_id'];
		$commentClass = $item::$commentClass;
		$contentField = $commentClass::$databaseColumnMap['content'];

		if ( $item::$firstCommentRequired AND isset( $item->$field ) AND isset( \IPS\Request::i()->$contentField ) )
		{
				$comment = $commentClass::load( $item->$field );
				$comment->$contentField = \IPS\Request::i()->$contentField;
				$comment->save();
	...
	protected function _create( \IPS\Node\Model $container = NULL, \IPS\Member $author, $firstPostParam = 'post' )
	{
		...
		{
			$commentClass = $item::$commentClass;
			$post = $commentClass::create( $item, \IPS\Request::i()->$firstPostParam, TRUE, $author->member_id ? NULL : $author->real_name, NULL, $author, $date );
