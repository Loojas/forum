...
abstract class _Item extends \IPS\Content
{
	...
	protected function processAfterCreate( $comment, $values )
	{
		if ( $comment !== NULL )
		{
			/* Check the content and see if we need to mod queue this for tripping any filters that require moderation. The Parser will handle any that are simply swapped out. */
			$hiddenByFilter = FALSE;
			if ( $this instanceof \IPS\Content\Hideable AND !$comment->author()->group['g_bypass_badwords'] )
			{
				$hiddenByFilter = \IPS\core\Profanity::hiddenByFilters( $comment->content() );
			}

			/* Was it hidden by filters? */
		...
		else
		{
			if ( $queryFlags & static::SELECT_IDS_FIRST or $groupBy )
			{
				...
				if ( $pass === true )
				{
					...
					{
						/* Reset the where */
						$where = array( array( \IPS\Db::i()->in( static::$databasePrefix . static::$databaseColumnId, $ids ) ) );
		...
		if ( in_array( 'IPS\Content\Hideable', class_implements( $class ) ) )
		{
			...
			if ( !$includeHidden )
			{
				...
				elseif( isset( $class::$databaseColumnMap['hidden'] ) )
				{
					...
					/* Possible values for this column are -1, 0, 1, 2. We want to select 0 and 2. However, when we use "OR", this can force MySQL to stop using indexes correctly. This is true of forums, for example. Using AND allows the index to be used. */
					$hiddenWhereClause = "({$col} != -1 AND {$col} != 1)";
			...
			if ( $values = $form->values() )
			{
				...
				if ( \IPS\Request::i()->isAjax() )
				{
					...
					else
					{
						...
						\IPS\Output::i()->json( array( 'type' => 'add', 'id' => $comment->$commentIdColumn, 'page' => $newPageCount, 'total' => $this->mapped('num_comments'), 'content' => $output, 'message' => $message ) );
					}
					return;
				}
				else
	...
	{
		return \IPS\Theme::i()->getTemplate( 'global', 'core' )->guestCommentTeaser( $this, new \IPS\Login( \IPS\Http\Url::internal( 'app=core&module=system&controller=login', 'front', 'login', NULL, \IPS\Settings::i()->logins_over_https ) ), $isReview );
	}
	...
	{
		$member = $member ?: \IPS\Member::loggedIn();
		if ( $member->member_group_id == \IPS\Settings::i()->guest_group or $member->restrict_post or ( $this instanceof \IPS\Content\Lockable and $this->locked() and ( !$member->member_id or !static::modPermission( 'reply_to_locked', $member, $this->container() ) ) ) )
		{
			return FALSE;
		}
	...
	public function canLock( $member=NULL )
	{
		...
		if( $member->member_id and static::modPermission( 'lock', $member, $container ) )
		{
			return TRUE;
		}

		if( $member->group['gbw_lock_unlock_own'] AND $member->member_id == $this->author()->member_id )
		{
			return TRUE;
		}
	...
	public function canUnlock( $member=NULL )
	{
		...
		if( $member->member_id and static::modPermission( 'unlock', $member, $container ) )
		{
			return TRUE;
		}
