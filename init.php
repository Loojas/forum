...
{	
	...
	public static function defaultConstants()
	{
		return array(
			...
			'SUITE_UNIQUE_KEY'					=> mb_substr( md5( '13_wf' . '$Rev: 3023$'), 10, 10 ),
	...
	public static function exceptionHandler( $exception )
	{
		...
		try
		{
			if ( !\IPS\IN_DEV and \IPS\Settings::i()->diagnostics_reporting and !\IPS\Settings::i()->theme_designers_mode and !self::exceptionWasThrownByThirdParty( $exception ) )
			{
	...
	final public static function reportExceptionToIPS( $exception )
	{
		...
		{
			\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 0 ), array( 'conf_key=?', 'diagnostics_reporting' ) );
	 ...
	 *
	 * @param	\Throwable			$exception	The exception
	 * @return	string|int|NULL		string = application directory, int = hook id, null means, that it was probably caused by an IPS4 application
	 */
	...
	{		
		...
		if ( preg_match( '/init\.php\(\d*\) : eval\(\)\'d code$/', $exception->getFile() ) )
		{
			...
			if ( preg_match( '/([a-zA-Z]+)_hook/', $trace, $matches ) )
			{
			...
			else if ( preg_match( '/hook(\d)/', $trace, $matches ) )
			{
		...
		{
			if ( preg_match( '/' . preg_quote( DIRECTORY_SEPARATOR, '/' ) . 'applications' . preg_quote( DIRECTORY_SEPARATOR, '/' ) . '([a-zA-Z]+)/', str_replace( \IPS\ROOT_PATH, '', $exception->getFile() ), $matches ) )
			{
				if ( !in_array( $matches[1], \IPS\Application::$ipsApps ) )
				{
					return $matches[1];
	 ...
	 * @return	void
	 */
	public static function resyncIPSCloud()
	{
		if ( \IPS\CIC )
		{
	...
	/**
	 * Get CiC Site Tracking ID
	 *
	 * @return	int|null
	 */
	public static function cicTrackingId()
	{
		if ( \IPS\CIC and preg_match( '/^\/var\/www\/html\/(.+?)(?:\/|$)/i', \IPS\ROOT_PATH, $matches ) )
		{
			$id = NULL;
			if ( isset( \IPS\Data\Store::i()->cicTrackingId ) )
			{
				$decoded = json_decode( \IPS\Data\Store::i()->cicTrackingId, TRUE );
				$id = $decoded['value'];
				if ( $decoded['time'] > ( time() - 86400 ) )
				{
					return $id;
				}
			}
			...
			try
			{
				$id = intval( (string) \IPS\Http\Url::external("https://invisioncloudstats.com/IPS/")->setQueryString( 'user', $matches[1] )->request()->get() );
			}
			catch ( \Exception $e )
			{
				return $id;
			...
			\IPS\Data\Store::i()->cicTrackingId = json_encode( array( 'time' => time(), 'value' => $id ) );
