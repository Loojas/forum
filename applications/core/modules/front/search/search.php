...
class _search extends \IPS\Dispatcher\Controller
{
	...
	protected function _results()
	{
		...
		{	
			...
			else
			{
				...
				if ( isset( \IPS\Request::i()->nodes ) )
				{
					...
					foreach ( explode( ',', \IPS\Request::i()->nodes ) as $id )
					{
						try
						{
		...
		else if( $count and ( $page < 1 or $page > $pages ) )
		{
			...
			\IPS\Output::i()->redirect( $baseUrl->setQueryString( 'page', 1 ) );
		}
	...
	public function _form()
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'tags', \IPS\Request::i()->tags, FALSE, array( 'autocomplete' => TRUE ), NULL, NULL, NULL, 'tags' ), NULL, 'search_tab_content' );
		...
		foreach ( $contentTypes as $k => $class )
		{
			if ( isset( $class::$containerNodeClass ) )
			{
				...
				$field = new \IPS\Helpers\Form\Node( $k . '_node', ( isset( \IPS\Request::i()->nodes ) ) ? \IPS\Request::i()->nodes : NULL, FALSE, array( 'class' => $nodeClass, 'multiple' => TRUE, 'permissionCheck' => $nodeClass::searchableNodesPermission(), 'forceOwner' => FALSE ), NULL, NULL, NULL, $k . '_node' );
