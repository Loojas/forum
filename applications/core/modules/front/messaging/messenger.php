...
class _messenger extends \IPS\Content\Controller
{
	...
	{		
		...
		{
			\IPS\Output::i()->error( \IPS\Member::loggedIn()->language()->addToStack('messenger_disabled_can_enable', FALSE, array( 'sprintf' => array( \IPS\Http\URL::internal( 'app=core&module=messaging&controller=messenger&do=enableMessenger', 'front', 'messaging' )->csrf() ) ) ), '2C137/A', 403, '' );
		}
	...
	protected function manage()
	{
		...
		if ( \IPS\Request::i()->q )
		{
			...
			if ( isset( \IPS\Request::i()->search ) and $searchValues = \IPS\Request::i()->valueFromArray('search') )
			{
				...
				{
					$query[] = \IPS\Content\Search\Mysql\Query::matchClause( "core_message_topics.mt_title", \IPS\Request::i()->q, '+', FALSE );
				}
				if ( isset( $searchValues['post'] ) )
				{
					$query[] = \IPS\Content\Search\Mysql\Query::matchClause( "core_message_posts.msg_post", \IPS\Request::i()->q, '+', FALSE );
				}
				if ( isset( $searchValues['recipient'] ) )
				{
					$query[] = "receiving_member.name LIKE '%" . \IPS\Db::i()->escape_string( \IPS\Request::i()->q ) . "%'";
				}
				if ( isset( $searchValues['sender'] ) )
				{
					$query[] = "sending_member.name LIKE '%" . \IPS\Db::i()->escape_string( \IPS\Request::i()->q ) . "%'";
				}
		...
		if ( \IPS\Request::i()->isAjax() )
		{
			...
			elseif( \IPS\Request::i()->overview )
			{
				\IPS\Output::i()->json( array( 
					'data' => \IPS\Theme::i()->getTemplate('messaging')->messageListRows( $conversations, $pagination, TRUE, $folders ),
	...
	protected function moveForm()
	{
		...
		$form->add( new \IPS\Helpers\Form\Select( 'move_message_to', NULL, FALSE, array( 'options' => array_merge( array( 'myconvo' => 'inbox' ), $folders ) ) ) );
		...
		if ( $values = $form->values() )
		{
			...
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=messaging&controller=messenger&folder=' . \IPS\Request::i()->move_message_to, 'front', 'messenger' ) );
		}
	...
	protected function addParticipant()
	{
		...
		try
		{
			...
			foreach ( $members as $member )
			{
				...
				if ( !$conversation::memberCanReceiveNewMessage( $member, \IPS\Member::loggedIn() ) )
				{
