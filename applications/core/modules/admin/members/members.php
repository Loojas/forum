...
class _members extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		$table->joins = array(
			array(
				'select' => 'v.vid, v.coppa_user, v.lost_pass, v.forgot_security, v.new_reg, v.email_chg, v.user_verified, v.spam_flag',
		...
		$table->parsers = array(
			...
			'name' => function( $val, $row )
			{
				...
				if ( $banned = $member->isBanned() )
				{
					...
					return "<a href='" . \IPS\Http\Url::internal( 'app=core&module=members&controller=members&do=edit&id=' ) . $row['member_id'] .  "'>" . htmlentities( $val, \IPS\HTMLENTITIES, 'UTF-8', FALSE ) . "</a> <span class='ipsBadge ipsBadge_negative'>" . $title ."</span> ";
				}
				else
				{
		...
		$table->rowButtons = function( $row )
		{
			...
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'members', 'member_delete' ) and ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'members', 'member_delete_admin' ) or !$member->isAdmin() ) and $member->member_id != \IPS\Member::loggedIn()->member_id )
			{
				$return['delete'] = array(
					...
					'data'      => array(
						'delete' => '',
						'delete-message' => \IPS\Member::loggedIn()->language()->addToStack('member_delete_message', FALSE, array( 'sprintf' => array( htmlentities( $member->name, \IPS\HTMLENTITIES, 'UTF-8', FALSE ) ) ) ),
	...
	public function massManage()
	{
		...
		{
			$start  = new \IPS\DateTime( \IPS\Helpers\Form\Date::_convertDateFormat( \IPS\Request::i()->members_joined['start'] ) );
			$start  = $start->setTime( 0, 0, 0 );

			$end    = new \IPS\DateTime( \IPS\Helpers\Form\Date::_convertDateFormat( \IPS\Request::i()->members_joined['end'] ) );
		...
		{
			$start  = new \IPS\DateTime( \IPS\Helpers\Form\Date::_convertDateFormat( \IPS\Request::i()->members_last_post['start'] ) );
			$start  = $start->setTime( 0, 0, 0 );

			$end    = new \IPS\DateTime( \IPS\Helpers\Form\Date::_convertDateFormat( \IPS\Request::i()->members_last_post['end'] ) );
			$end    = $end->setTime( 23, 59, 59 );
		...
		{
			$start  = new \IPS\DateTime( \IPS\Helpers\Form\Date::_convertDateFormat( \IPS\Request::i()->members_last_visit['start'] ) );
			$start  = $start->setTime( 0, 0, 0 );

			$end    = new \IPS\DateTime( \IPS\Helpers\Form\Date::_convertDateFormat( \IPS\Request::i()->members_last_visit['end'] ) );
			$end    = $end->setTime( 23, 59, 59 );
	...
	public function doMove()
	{
		...
		{
			\IPS\Task::queue( 'core', 'MoveMembers', array( 'where' => $_SESSION['members_manage_where'], 'group' => $_SESSION['members_manage_group'], 'oldGroup' => ( isset( $_SESSION['members_manage_old_group'] ) ) ? $_SESSION['members_manage_old_group'] : NULL ), 2 );
	...
	public function edit()
	{
		...
		if ( $values = $form->values() )
		{
			...
			/* Has the member been promoted? */
			$redirectMessage = ( $member->member_group_id != $values['group'] ) ? \IPS\Member::loggedIn()->language()->addToStack('member_edit_promoted', FALSE, array( 'sprintf' => array( $member->groupName ) ) ) : 'saved';

			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=members&controller=members&do=edit&id=' . $member->member_id ), $redirectMessage );
		}
	...
	protected function password()
	{
		...
		/* Show Form */
		$form = new \IPS\Helpers\Form;
		$form->add( new \IPS\Helpers\Form\Password( 'password', '', TRUE, array( 'confirm' => 'password_confirm', 'showMeter' => \IPS\Settings::i()->password_strength_meter, 'minimumStrength' => 1 ) ) );
	...
	public function ip()
	{
		...
		/* Display */
		\IPS\Output::i()->title			= $member->name;
		\IPS\Output::i()->breadcrumb[]	= array( \IPS\Http\Url::internal("app=core&module=members&controller=members&do=edit")->setQueryString("id", $member->member_id), $member->name );
	...
	public function delete()
	{
		...
		\IPS\Output::i()->title = \IPS\Member::loggedIn()->language()->addToStack( 'member_deletion_title' );
		\IPS\Output::i()->output = $form;
	}
	...
	protected function adminPassword()
	{
		...
		$form->add( new \IPS\Helpers\Form\Password( 'new_password', '', TRUE, array( 'showMeter' => \IPS\Settings::i()->password_strength_meter, 'checkStrength' => TRUE ) ) );
