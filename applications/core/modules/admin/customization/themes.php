...
class _themes extends \IPS\Node\Controller
{
	...
	public function add()
	{
		...
		if ( $values = $form->values() )
		{
			if ( isset( $values['core_theme_set_new_import'] ) and $values['core_theme_set_new_import'] )
			{
				...
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=customization&controller=themes&do=import' )->setQueryString( array( 'file' => $tempFile, 'key' => md5_file( $tempFile ), 'id' => $theme->id ) ) );
			}
			else
			{
	...
	public function import()
	{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			\IPS\Http\Url::internal( 'app=core&module=customization&controller=themes&do=import' )->setQueryString( array( 'file' => \IPS\Request::i()->file, 'key' =>  \IPS\Request::i()->key, 'id' => \IPS\Request::i()->id ) ),
			function( $data )
			{
										...
										switch ( $xml->name )
										{
											...
											case 'css':
												...
												else
												{
													...
													if ( in_array( $name, $masterCss[ $appKey ][ $location ][ $path ] ) )
													{
														$edited = time();
														$addedTo = 0;
													}
	...
	public function cssForm()
	{
		...
		function( $val )
		{
			...
			try
			{
				$row = \IPS\Db::i()->select( '*', 'core_theme_css', array( \IPS\Db::i()->in( 'css_set_id', array( 0, \IPS\Request::i()->id ) ) . " AND css_app=? AND css_path=? AND css_location=? AND css_name=?", \IPS\Request::i()->theme_css_app, $path, $location, $val ) )->first();
	...
	public function importForm()
	{
		...
		if ( $values = $form->values() )
		{
			...
			/* Initate a redirector */
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=customization&controller=themes&do=import' )->setQueryString( array( 'file' => $tempFile, 'key' => md5_file( $tempFile ), 'id' => \IPS\Request::i()->id) ) );
		}
	...
	public function exportForm()
	{
		...
		if ( \IPS\IN_DEV OR \IPS\Theme::designersModeEnabled() )
		{
			...
			{
				$url = \IPS\Http\Url::internal( 'app=core&module=customization&controller=themes&do=devImportBeforeExport&id=' . $id );
		...
		if ( $values = $form->values() or \IPS\Request::i()->form_submitted )
		{
			...
			\IPS\Output::i()->sendOutput( $xml->outputMemory(), 200, 'application/xml', array( 'Content-Disposition' => \IPS\Output::getContentDisposition( 'attachment', $name ), FALSE, FALSE ) );
		}
