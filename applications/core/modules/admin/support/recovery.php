...
class _recovery extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect( \IPS\Http\Url::internal( 'app=core&module=support&controller=recovery' ), function( $step )
		{
			$step = intval( $step );
			...
			switch( $step )
			{
				...
				case 3: # Login Handlers
					$handlersDisabled = array();
					
					/* Disable all login handlers other than Internal */
					foreach( \IPS\Login::handlers( TRUE ) AS $key => $handler )
					{
						if ( $key != 'Internal' )
						{
							...
							$handlersDisabled[] = $key;
						}
					}
					
					$_SESSION['recoveryLogins'] = $handlersDisabled;
					
					return array( 4, \IPS\Member::loggedIn()->language()->addToStack( 'disabled_login_handlers' ), 100 );
					break;
	...
	public function done()
	{
		...
		/* Did we disable any login handlers? */
		$logins = array();
		foreach( $_SESSION['recoveryLogins'] AS $handler )
		{
			$logins[] = \IPS\Login\LoginAbstract::constructFromData( \IPS\Db::i()->select( '*', 'core_login_handlers', array( "login_key=?", $handler ) )->first() );
		}
		
		\IPS\Output::i()->title = \IPS\Member::loggedIn()->language()->addToStack( 'recovery_mode' );
		\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'support' )->recovery( $apps, $plugins, $theme, $logins );
	}
}