...
{	
	...
	public function sendImmediately()
	{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			...
			function( $data )
			{
				...
				if ( !is_array( $data ) )
				{
					return array( array( 'total' => $mail->getQuery( \IPS\core\BulkMail\Bulkmailer::GET_COUNT_ONLY )->first() ), 
				...
				{
					return array( $data, 
						\IPS\Member::loggedIn()->language()->addToStack('bulkmail_sent_sofar', FALSE, array( 'sprintf' => array( $mail->subject, \IPS\Member::loggedIn()->language()->addToStack( 'bm_users', FALSE, array( 'pluralize' => array( $mail->sentto ) ) ) ) ) ),
						min( ( 100 / $data['total'] * $mail->offset ), 100 )
					);
				}
			},
			function()
	...
	public function form()
	{
		...
		$form->add( new \IPS\Helpers\Form\Editor( 'mail_body', $mail->content ?: '', TRUE, array( 'app' => 'core', 'key' => 'Admin', 'autoSaveKey' => 'bulkmail' . ( $mail->id ? '-' . $mail->id : '' ), 'tags' => $tags, 'attachIds' => array( $mail->id, NULL, 'bulkmail' ) ) ) );
