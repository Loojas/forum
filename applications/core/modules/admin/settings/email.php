...
class _email extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		$disabled = array();
		if ( \IPS\Settings::i()->sparkpost_api_key )
		{
			if ( \IPS\Settings::i()->sparkpost_use_for == 2 )
			{
				$method = 'sparkpost';
			}
		}
		...
		$settingsForm->add( new \IPS\Helpers\Form\Radio( 'mail_method', $method, TRUE, array(
			'options' 	=> array( 'php' => ( \IPS\CIC ? 'mail_method_cic' : 'mail_method_php' ), 'smtp' => 'mail_method_smtp', 'sendgrid' => 'mail_method_sendgrid', 'sparkpost' => 'mail_method_sparkpost' ),
			'disabled'	=> $disabled,
			'toggles' => array(
		...
		if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'settings', 'email_test' ) )
		{
			...
			if ( $values = $testForm->values() )
			{
				...
				{
					$messageHtml = \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->message( get_class( $exception ) . ": " . $exception->getMessage() . " (" . $exception->getCode() . ")", 'error' );
				}
			}
