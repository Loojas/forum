...
class _posting extends \IPS\Dispatcher\Controller
{
	...
	public function profanity()
	{
		...
		if ( $values = $form->values() )
		{
			...
			if ( isset( $values['profanity_upload'] ) and $values['profanity_upload'] )
			{
				...
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=settings&controller=posting&do=importProfanity' )->setQueryString( array( 'file' => $tempFile, 'key' => md5_file( $tempFile ) ) ) );
			}
	...
	protected function importProfanity()
	{
		...
		$url = \IPS\Http\Url::internal( 'app=core&module=settings&controller=posting&do=importProfanity' )->setQueryString( array( 'file' => \IPS\Request::i()->file, 'key' =>  \IPS\Request::i()->key ) );
