		...
		foreach ( parent::formElements( $item, $container ) as $k => $v )
		{
 				...
 				if( !$item )
 				{
					...
					$return['to'] = new \IPS\Helpers\Form\Member( 'messenger_to', $member, TRUE, array( 'multiple' => ( \IPS\Member::loggedIn()->group['g_max_mass_pm'] == -1 ) ? NULL : \IPS\Member::loggedIn()->group['g_max_mass_pm'] ), function ( $members )
					{
						if ( is_array( $members ) )
						{
							...
							{
								if ( !$m instanceof \IPS\Member OR !static::memberCanReceiveNewMessage( $m, \IPS\Member::loggedIn() ) )
								{
						...
						{
							if ( !$members instanceof \IPS\Member OR !$members->member_id OR !static::memberCanReceiveNewMessage( $members, \IPS\Member::loggedIn() ) )
							{
			...
			else if ( $k == 'content' )
			{
				...
				else
				{
					...
					$v->options['autoSaveKey'] = 'newMessageTo-' . uniqid();
				}
			}
