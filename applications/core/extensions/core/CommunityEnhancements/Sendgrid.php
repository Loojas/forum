...
class _SendGrid
{
	...
	public function toggle( $enabled )
	{
		...
		if( !$enabled )
		{
			\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 0 ), array( 'conf_key=?', 'sendgrid_use_for' ) );
		...
		{
			\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => 1 ), array( 'conf_key=?', 'sendgrid_use_for' ) );
			unset( \IPS\Data\Store::i()->settings );
		}
		else
		{
