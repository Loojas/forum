...
class _Content
{
	...
	public function process( &$form, $group )
	{
		...
		if( $group->g_id != \IPS\Settings::i()->guest_group )
		{
			$form->add( new \IPS\Helpers\Form\YesNo( 'gbw_lock_unlock_own', $group->g_bitoptions['gbw_lock_unlock_own'] ) );
		}
	...
	public function save( $values, &$group )
	{
		...
		$bwKeys = array( 'gbw_disable_tagging', 'gbw_disable_prefixes', 'gbw_soft_delete_own', 'gbw_no_report', 'gbw_delete_attachments', 'gbw_post_highlight', 'gbw_lock_unlock_own' );
		foreach ( $bwKeys as $k )
		{
			if ( isset( $values[ $k ] ) )
			{
