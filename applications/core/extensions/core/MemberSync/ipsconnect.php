...
class _ipsconnect
{
	...
	public function onCreateAccount( $member )
	{
		...
		/* We can only create accounts on the master if we have all the details */
		if( !$member->name OR !$member->email OR !$member->members_pass_hash OR !$member->members_pass_salt )
		{
			return;
		}
