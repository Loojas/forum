...
class _Bulkmail
{
	...
	public function run( $data, $offset )
	{
		try
		{
			...
			$results = $mail->getQuery( array( $offset, $classToUse::MAX_EMAILS_PER_GO ) );
			...
			/* Send it */
			$email = \IPS\Email::buildFromContent( $mail->subject, $content, NULL, \IPS\Email::TYPE_BULK )
			...
			$mail->updated	= time();
			$mail->offset	= ( $mail->offset + $classToUse::MAX_EMAILS_PER_GO );
			$mail->sentto	= ( $mail->sentto + $sent );
			$mail->save();
			
			return $offset + $classToUse::MAX_EMAILS_PER_GO;
		}
