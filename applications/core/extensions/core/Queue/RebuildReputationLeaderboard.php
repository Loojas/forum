...
class _RebuildReputationLeaderboard
{
	...
	public function run( &$data, $offset )
	{
		...
		for( $i = 0; $i < $this->rebuild; $i++ )
		{
			...
			foreach( \IPS\Db::i()->select( 'core_reputation_index.member_received as member, SUM(rep_rating) as rep', 'core_reputation_index', array( 'member_received > 0 AND rep_date BETWEEN ? and ?', $start->getTimeStamp(), $end->getTimeStamp() ), 'rep DESC', 4, 'member' )->setKeyField('member')->setValueField('rep') as $member => $rep )
			{
				if ( $member and $rep )
				{
