...
\IPS\Content\Embeddable
{
	...
	public function customFields( $topic = FALSE )
	{
		...
		{
			$this->_fieldData	= iterator_to_array( \IPS\Db::i()->select( 'cf_id,cf_format', 'downloads_cfields', array( 'cf_topic=?', 1 ) )->setKeyField( 'cf_id' )->setValueField( 'cf_format' ) );
		}
		...
		try
		{
			...
			foreach ( $this->_customFields as $k => $v )
			{
				...
				if( $topic === TRUE and !isset( $this->_fieldData[ str_replace( 'field_', '', $k ) ] ) )
				{
					continue;
				}
				...
				if ( array_key_exists( str_replace( 'field_', '', $k ), $fields ) )
				{
					if( $topic === TRUE )
					{
						if( isset( $this->_fieldData[ str_replace( 'field_', '', $k ) ] ) )
						{
							$v	= str_replace( '{content}', htmlspecialchars( $v, \IPS\HTMLENTITIES, 'UTF-8', FALSE ), $this->_fieldData[ str_replace( 'field_', '', $k ) ] );
	...
	public function downloadCheck( array $record = NULL, \IPS\Member $member = NULL )
	{
		...
		/* Paid? */
		if ( $this->isPaid() and !$member->group['idm_bypass_paid'] and $member->member_id != $this->author()->member_id )
		{
			if ( $this->cost )
			{
	...
	public static function formElements( $item=NULL, \IPS\Node\Model $container=NULL )
	{
		...
		/* Description */
		$return['description'] = new \IPS\Helpers\Form\Editor( 'file_desc', $item ? $item->desc : NULL, TRUE, array( 'app' => 'downloads', 'key' => 'Downloads', 'autoSaveKey' => 'downloads-new-file', 'attachIds' => ( $item === NULL ? NULL : array( $item->id, NULL, 'desc' ) ) ) );
