...
class _Category extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	public function set__enabled( $enabled )
	{
		...
		if ( $enabled )
		{
			\IPS\Content\Search\Index::i()->massUpdate( static::$contentItemClass, $this->_id, NULL, $this->searchIndexPermissions() );
		}
		else
		{
			\IPS\Content\Search\Index::i()->massUpdate( static::$contentItemClass, $this->_id, NULL, '' );
		}
	}
	...
	protected function get_cfields()
	{
		if ( $this->_customFields === NULL )
		{
			...
			{
				foreach( new \IPS\Patterns\ActiveRecordIterator( \IPS\Db::i()->select( '*', 'downloads_cfields', array( \IPS\Db::i()->in( 'cf_id', explode( ',', $this->_data['cfields'] ) ) ) ), 'IPS\downloads\Field' ) AS $field )
				{
