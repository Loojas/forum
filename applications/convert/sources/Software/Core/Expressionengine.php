...
class _Expressionengine extends \IPS\convert\Software
{
	...
	public function convertAttachments()
	{
		...
		foreach( $it AS $row )
		{
			...
			try
			{
				if ( $attachId !== FALSE )
				{
					$ourAttachment = \IPS\File::get( 'core_Signature', \IPS\Db::i()->select( 'attach_location', 'core_attachments', array( 'attach_id=?', $attachId ) )->first() );
					...
					$signature .= '<p><a href="<fileStore.core_Signature>/' . (string) $ourAttachment . '" class="ipsAttachLink ipsAttachLink_image"><img data-fileid="' . $attachId . '" src="<fileStore.core_Signature>/' . (string) $ourAttachment . '" class="ipsImage ipsImage_thumbnailed" alt=""></a></p>';
	...
	public function convertProfileFields()
	{
		...
		foreach( $this->fetch( 'exp_member_fields', 'm_field_id' ) AS $row )
		{
			...
			$info['pf_member_hide']		= $row['m_field_public'] == 'n' ? 1 : 0;
	...
	public function convertRanks()
	{
		...
		foreach( $this->fetch( 'exp_forum_ranks', 'rank_id' ) AS $row )
		{
			$libraryClass->convertRank( array(
				'id'		=> $row['rank_id'],
				'title'		=> $row['rank_title'],
				'posts'		=> $row['rank_min_posts'],
				'pips'		=> $row['rank_stars']
			) );
		...
		foreach( $it AS $row )
		{
			...
			try
			{
				if ( $attachId !== FALSE )
				{
					...
					$message .= '<p><a href="<fileStore.core_Attachment>/' . (string) $ourAttachment . '" class="ipsAttachLink ipsAttachLink_image"><img data-fileid="' . $attachId . '" src="<fileStore.core_Attachment>/' . (string) $ourAttachment . '" class="ipsImage ipsImage_thumbnailed" alt=""></a></p>';
