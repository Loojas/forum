...
class _Smf extends \IPS\convert\Software
{
		...
		switch( $method )
		{
			...
			case 'convertMembers':
				...
				$return['convertMembers']['username_or_display_name'] = array(
					...
					'field_required'		=> TRUE,
					'field_extra'			=> array( 'options' => array( 'user_name', 'display_name' ) ),
					'field_hint'			=> NULL,
				);
	...
	public static function fixPostData( $post )
	{
		...
		// And img tags
		$post = preg_replace("#\[img width=(\d+) height=(\d+)\]([a-zA-Z0-9.:\-/]+)\[\/img\]#i", "[img]$3[/img]", $post);
	...
	public function convertMembers()
	{
		...
		foreach( $this->fetch( 'members', 'id_member' ) AS $row )
		{
			...
			$info = array(
				...
				'name'					=> $name,
				'password'				=> $row['passwd'],
				'password_extra'		=> $row['password_salt'],
