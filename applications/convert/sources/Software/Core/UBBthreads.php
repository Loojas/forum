...
class _UBBthreads extends \IPS\convert\Software
{
	...
	public function convertMembers()
	{
		...
		foreach ( $select as $row )
		{
			...
			$libraryClass->convertMember( $info, $pfields, $profilePhotoName, NULL, $profilePhotoData );
			$libraryClass->setLastKeyValue( $row['USER_ID'] );
		}
	}
	...
	public function convertMembersFollowers()
	{
		...
		foreach ( $this->fetch( 'WATCH_LISTS', 'WATCH_ID', array( 'WATCH_TYPE=?', 'u' ), 'USER_ID ASC, WATCH_ID ASC' ) as $row )
		{
