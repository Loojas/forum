...
class _UBBthreads extends \IPS\convert\Software
{
	...
	public function convertForumsForumsFollowers()
	{
		...
		foreach ( $this->fetch( 'WATCH_LISTS', 'WATCH_ID', array( 'WATCH_TYPE=?', 'f' ), 'USER_ID ASC, WATCH_ID ASC' ) as $row )
		{
	...
	public function convertForumsTopicsFollowers()
	{
		...
		foreach ( $this->fetch( 'WATCH_LISTS', 'WATCH_ID', array( 'WATCH_TYPE=?', 't' ), 'USER_ID ASC, WATCH_ID ASC' ) as $row )
		{
			$libraryClass->convertFollow( array(
				'follow_app'            => 'forums',
				'follow_area'           => 'topic',
	...
	public function checkRedirects()
	{
		...
		if( preg_match( '#/ubbthreads.php/topics/([0-9]+)($|/)#i', $url->data[ \IPS\Http\Url::COMPONENT_PATH ], $matches ) )
		{
			try
			{
				try
