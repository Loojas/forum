...
class _category extends \IPS\cms\Databases\Controller
{
	...
	public function view()
	{
		...
		{
			if ( $table->sortBy == 'field_' . $id )
			{
				if ( $obj->type == 'Number' )
				{
					$table->sortOptions['field_' . $id ] = 'LENGTH(`field_' . $id . '`) ' . $table->sortDirection . ',`field_' . $id . '`';
					break;
				}
				elseif( $obj->type == 'Date' )
				{
					$table->sortOptions['field_' . $id ] = 'CAST(`field_' . $id . '` AS UNSIGNED)';
					break;
				}
			}
		}
		...
		$table->advancedSearchCallback = function( $table, $values ) use ( $database, $sortBy, $customFields )
		{
			...
			if ( isset( $values['sort_by'] ) and isset( $sortBy[ $values['sort_by'] ] ) )
			{
				...
				{
					$table->sortBy = 'LENGTH(`' . $values['sort_by'] . '`) ' . $table->sortDirection . ',`' . $values['sort_by'] . '`';
				}
				elseif( isset( $customFields[ mb_substr( $values['sort_by'], 6 ) ] ) and $customFields[ mb_substr( $values['sort_by'], 6 ) ]->type == 'Date' )
				{
					$table->sortBy = 'CAST(`' . $values['sort_by'] . '` AS UNSIGNED)';
				}
				else
				{
