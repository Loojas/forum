...
class _databases extends \IPS\Node\Controller
{
	...
	public function form()
	{
		...
		$fields = array(
			...
			'record_last_comment' => ( $current and $current->_comment_bump === \IPS\cms\Databases::BUMP_ON_EDIT ) ? 'database_field__edited' : 'database_field__updated',
		...
		if ( \IPS\Application::appIsEnabled( 'forums' ) )
		{
			...
			if ( $current )
			{
				...
				\IPS\Member::loggedIn()->language()->words['database_forum_record_desc'] = \IPS\Member::loggedIn()->language()->addToStack( 'database_forum_record__desc' ) . ' ' . \IPS\Member::loggedIn()->language()->addToStack( 'database_forum_record__rebuild', NULL, array( 'sprintf' => array( $rebuildUrl ) ) );
			}
			else
			{
