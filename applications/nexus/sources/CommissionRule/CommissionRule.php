...
class _CommissionRule extends \IPS\Node\Model
{
	...
	public function form( &$form )
	{
		$numberOfPurchasesField = array(
			...
			{
				return \IPS\Theme::i()->getTemplate('customers')->numberOfPurchasesField( $field );
			},
		...
		$form->add( new \IPS\Helpers\Form\Select( 'rrule_by_group', ( $this->by_group and $this->by_group != '*' ) ? explode( ',', $this->by_group ) : '*', FALSE, array( 'options' => $groupsExcludingGuests, 'multiple' => TRUE, 'unlimited' => '*', 'unlimitedLang' => 'any' ) ) );
		...
		$form->add( new \IPS\Helpers\Form\Select( 'rrule_for_group', ( $this->for_group and $this->for_group != '*' ) ? explode( ',', $this->for_group ) : '*', FALSE, array( 'options' => $groups, 'multiple' => TRUE, 'unlimited' => '*', 'unlimitedLang' => 'any' ) ) );
		...
		$form->add( new \IPS\Helpers\Form\Custom( 'rrule_purchase_amount', array( $this->purchase_amount_op, $this->purchase_amount_unit ), FALSE, array(
			'getHtml'	=> function( $field )
			{
				return \IPS\Theme::i()->getTemplate('customers')->purchaseValueField( $field );
			},
			'formatValue'	=> function( $field )
			{
				$value = $field->value;
