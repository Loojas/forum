							...
							foreach ( $val['other'] as $change )
							{
								...
								if( mb_strpos( $change['name'], 'nexus_ccfield_' ) !== FALSE AND is_array( $change['value'] ) )
								{
									...
									catch( \BadFunctionCallException $e )
									{
										...
										$change['value'] = htmlspecialchars( $value, \IPS\HTMLENTITIES, 'UTF-8', FALSE );
									}
								}
					...
					case 'download':
						...
						switch ( $val['type'] )
						{
							case 'idm':
								try
								{
									if ( \IPS\Application::appIsEnabled( 'downloads' ) === FALSE )
									{
										/* If the app is disabled, bubble up to avoid an \IPS\Db\Exception for missing tables - we still want to show the log */
										throw new \OutOfRangeException;
									}
