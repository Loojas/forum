...
class _Purchase extends \IPS\Node\Model
{
	...
	public function acpSupportView()
	{
		try
		{
			return call_user_func( array( $this->extension(), 'acpSupportView' ), $this );
		}
		catch ( \OutOfRangeException $e )
		{
			return '';
	...
	public function renewalPaymentMethodIds()
	{
		try
		{
			return call_user_func( array( $this->extension(), 'renewalPaymentMethodIds' ), $this );
		}
		catch ( \OutOfRangeException $e )
		{
			return NULL;
	...
	public function apiOutput()
	{
		...
		if ( $this->parent )
		{
			try
			{
				$parent = static::load( $this->parent )->apiOutput();
			}
			catch ( \OutOfRangeException $e ) {}
		}
		...
		return array(
			...
			'renewalTerm'	=> $this->renewals ? $this->renewals->apiOutput() : null,
	...
	public function url()
	{
		if( $this->_url === NULL )
		{
			$this->_url = \IPS\Http\Url::internal( "app=nexus&module=clients&controller=purchases&do=view&id={$this->id}", 'front', 'clientspurchase', array( \IPS\Http\Url\Friendly::seoTitle( $this->name ) ), \IPS\Settings::i()->nexus_https );
		}
