...
abstract class _Item
{
	...
	{
		return new \IPS\Math\Number( $this->tax ? $this->tax->rate( $location ?: $this->billaddress ) : '0' );
	}
	 ...
	 * @apiresonse		float								commission		If the payment for this item goes to another user (for example for Downloads files), the percentage of the price that will be retained by the site (in addition to fee)
	...
	public function apiOutput()
	{
		return array(
			...
			'itemPrice'			=> $this->price->apiOutput(),
			'linePrice'			=> $this->linePrice()->apiOutput(),
			...
			'payTo'				=> $this->payTo ? $this->payTo->apiOutput() : null,
			'commission'		=> $this->commission,
			'fee'				=> $this->fee ? $this->fee->apiOutput() : null,
		);
	}
}