...
class _Invoice extends \IPS\Patterns\ActiveRecord
{
	...
	public function buttons( $ref='v' )
	{
		...
		if ( $this->status !== \IPS\nexus\Invoice::STATUS_PAID and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'payments', 'invoices_add' ) )
		{
			$return['edit'] = array(
				'title'		=> 'edit',
				'icon'		=> 'pencil',
	...
	public function apiOutput()
	{
		...
		{
			$tax[] = ( new \IPS\nexus\Invoice\Item\TaxItem( $classId, $data ) )->apiOutput();
		}
		...
		return array(
			...
			'items'				=> array_map( function( $item ){
				return $item->apiOutput();
			}, $summary['items'] ),
			'subTotal'			=> $summary['subtotal']->apiOutput(),
			'shippingCharges'	=> array_map( function( $item ){
				return $item->apiOutput();
			}, $summary['shipping'] ),
			'shippingTotal'		=> $summary['shippingTotal']->apiOutput(),
			'tax'				=> $tax,
			'taxTotal'			=> $summary['taxTotal']->apiOutput(),
			'total'				=> $summary['total']->apiOutput(),
			'outstanding'		=> $this->amountToPay()->apiOutput(),
			'billingAddress'	=> $this->billaddress ? $this->billaddress->apiOutput() : null,
			'shippingAddress'	=> $this->shipaddress ? $this->shipaddress->apiOutput() : null,
			'poNumber'			=> $this->po,
			'notes'				=> $this->notes,
			'transactions'		=> array_map( function( $transaction ){
				return $transaction->apiOutput();
			}, iterator_to_array( $this->transactions() ) ),
			'shipments'			=> array_map( function( $shipment ){
				return $shipment->apiOutput();
			}, iterator_to_array( $this->shipments() ) ),
			'purchases'			=> array_map( function( $purchase ){
				return $purchase->apiOutput();
	...
	public function markPaid( \IPS\Member $member = NULL )
	{
		...
		if ( $this->status !== static::STATUS_PAID )
		{
			...
			if ( !$this->member->member_id and $this->guest_data and $this->requiresLogin() )
			{
				...
				{
					$this->member->postRegistration( FALSE, TRUE );
				}
				else if( \IPS\Settings::i()->new_reg_notify )
				{
					\IPS\Email::buildFromTemplate( 'core', 'registration_notify', array( $memberToSave, $profileFields ), \IPS\Email::TYPE_LIST )->send( \IPS\Settings::i()->email_in );
				}
