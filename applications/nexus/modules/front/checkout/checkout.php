...
class _checkout extends \IPS\Dispatcher\Controller
{
	...
	public function execute()
	{
		...
		{
			\IPS\Output::i()->redirect( \IPS\Request::i()->url()->setScheme('https') );
		}
	...
	{		
		...
		try
		{
			...
			{
				if ( !$this->invoice->member->member_id and \IPS\Member::loggedIn()->member_id and isset( \IPS\Request::i()->loginKey ) and \IPS\Request::i()->loginKey === md5( json_encode( $this->invoice->items ) . session_id() . \IPS\SUITE_UNIQUE_KEY ) )
				{
					$this->_assignInvoiceToMember( \IPS\Member::loggedIn() );
				}
	...
	public function _customer()
	{
		...
		/* Billing Information */
		if ( $needBillingInfo or ( $this->needsToLogin and \IPS\Settings::i()->allow_reg and !\IPS\Settings::i()->nexus_checkreg_usernames ) )
		{
		...
		if ( $values = $form->values() )
		{
			...
			if ( $this->needsToLogin )
			{
				...
				if ( !$form->error )
				{
					...
					/* Save on invoice */
					$this->invoice->guest_data = array( 'member' => $member->changed, 'profileFields' => $profileFields, 'securityAnswers' => $securityQuestionAnswers );
				}
			}
			...
			else
			{
				...
				if ( $this->invoice->member->member_id )
				{
				...
				else
				{
					...
					$this->invoice->guest_data = array( 'member' => $this->invoice->member->changed, 'profileFields' => array(), 'securityAnswers' => array() );
				}
			}
		...
		if ( $this->needsToLogin )
		{
			$login = new \IPS\Login( $this->invoice->checkoutUrl()->setQueryString( 'loginKey', md5( json_encode( $this->invoice->items ) . session_id() . \IPS\SUITE_UNIQUE_KEY ) ) );
			$loginForms = $login->forms( FALSE, TRUE ); // This has to be before authenticate() as the result gets cached and we need to specify the destination is the same page
			try
			{
				$member = $login->authenticate();
				if ( $member !== NULL )
				{
					$this->_assignInvoiceToMember( $member, $login->flags['remember_me'], $login->flags['signin_anonymous'] );
				}
			}
	 ...
	 * @param	bool		$rememberMe	Set "Remember me" flag?
	 * @param	bool		$anonymous	Login is anonymous?
	 * @return	void
	 */
	protected function _assignInvoiceToMember( \IPS\Member $member, $rememberMe=TRUE, $anonymous=FALSE )
	{
	...
	public function _pay( $data )
	{
		...
		foreach ( $this->invoice->items as $item )
		{
			if ( $item->groupWithParent and is_int( $item->parent ) and isset( $item->renewalTerm ) and $item->renewalTerm )
			{
				...
				if ( isset( $parent->renewalTerm ) and $parent->renewalTerm )
				{
		...
		/* Display */
		return \IPS\Theme::i()->getTemplate('checkout')->confirmAndPay( $this->invoice, $this->invoice->summary(), $form->customTemplate( array( call_user_func_array( array( \IPS\Theme::i(), 'getTemplate' ), array( 'checkout', 'nexus' ) ), 'paymentForm' ), $this->invoice, $amountToPay ), $amountToPay, $couponForm ? $couponForm->customTemplate( array( call_user_func_array( array( \IPS\Theme::i(), 'getTemplate' ), array( 'checkout', 'nexus' ) ), 'couponForm' ) ) : NULL, $recurrings, $overriddenRenewalTerms );
	}
