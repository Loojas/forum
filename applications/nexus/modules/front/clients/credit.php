...
class _credit extends \IPS\Dispatcher\Controller
{
	...
	public function execute()
	{
		...
		if ( $output = \IPS\MFA\MFAHandler::accessToArea( 'nexus', 'AccountCredit', \IPS\Http\Url::internal( 'app=nexus&module=clients&controller=credit', 'front', 'clientscredit', array(), \IPS\Settings::i()->nexus_https ) ) )
		{
	...
	protected function manage()
	{
		...
		if ( count( $withdrawOptions ) )
		{
			...
			{
				if ( isset( $balance[ $currency ] ) and $balance[ $currency ]->amount->isGreaterThanZero() )
				{
					$canWithdraw = TRUE;
				}
			}
