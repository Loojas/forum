...
{	
	...
	protected function manage()
	{
		...
		/* Get my stream */
		$stream = NULL;
		if ( isset( \IPS\Request::i()->cookie['support_stream'] ) or isset( \IPS\Request::i()->stream ) or isset( \IPS\Request::i()->member ) )
		{
			if ( isset( \IPS\Request::i()->member ) )
			{
				try
				{
					...
					\IPS\Db::i()->delete( 'nexus_support_streams', array( 'stream_owner=? AND stream_temporary=1 AND stream_id<>?', \IPS\Member::loggedIn()->member_id, $stream->id ) );			
				}
			...
			{
				$streamId = isset( \IPS\Request::i()->stream ) ? \IPS\Request::i()->stream : \IPS\Request::i()->cookie['support_stream'];
				try
				{
					...
					{
						\IPS\Request::i()->setCookie( 'support_stream', $streamId, \IPS\DateTime::create()->add( new \DateInterval( 'P1Y') ), FALSE );
						\IPS\Db::i()->delete( 'nexus_support_streams', array( 'stream_owner=? AND stream_temporary=1 AND stream_id<>?', \IPS\Member::loggedIn()->member_id, $streamId ) );
					}
				}
				catch ( \OutOfRangeException $e ) { }
			}
		...
		if ( $values = $form->values() )
		{
			if ( $values['stream'] == 'custom' )
			{
				...
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal('app=nexus&module=support&controller=requests&stream=') . $stream->id );
			}
			else
			{
		...
		$form = $form->customTemplate( array( call_user_func_array( array( \IPS\Theme::i(), 'getTemplate' ), array( 'support' ) ), 'filterForm' ) );
	...
	{
		\IPS\Output::i()->title = \IPS\Member::loggedIn()->language()->addToStack('menu__nexus_support_requests');
		\IPS\Output::i()->output = \IPS\nexus\Support\Stream::load('open')->form( NULL, \IPS\Http\Url::internal('app=nexus&module=support&controller=requests') )->customTemplate( array( call_user_func_array( array( \IPS\Theme::i(), 'getTemplate' ), array( 'support' ) ), 'filterForm' ) );
	}
	...
	protected function editStream()
	{
		...
		\IPS\Output::i()->output = $form->customTemplate( array( call_user_func_array( array( \IPS\Theme::i(), 'getTemplate' ), array( 'support' ) ), 'filterForm' ) );
	}
	...
	protected function preferences()
	{
		...
		$form->add( new \IPS\Helpers\Form\Editor( 'nexus_default_support_content', $preferences ? $preferences['content'] : NULL, FALSE, array( 'app' => 'nexus', 'key' => 'Support', 'autoSaveKey' => 'default_content', 'tags' => array(
			...
			'{customer_last_name}'	=> \IPS\Member::loggedIn()->language()->addToStack('customer_first_name'),
	...
	protected function create()
	{
		...
		$steps['your_message'] = function( $data ) use ( $stockActions )
		{
			...
			try
			{
				...
				$defaultContent = str_replace(
					array( '{customer_first_name}', '{customer_last_name}', '{customer_full_name}', '{department_name}', '{department_email}' ),
					array(
			...
			$form->add( new \IPS\Helpers\Form\Editor( 'message', $stockAction ? $stockAction->message : $defaultContent, TRUE, array( 'app' => 'nexus', 'key' => 'Support', 'autoSaveKey' => "new-req-message", 'defaultIfNoAutoSave' => TRUE ) ) );
			if ( $values = $form->values() )
			{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\Wizard( $steps, $url, TRUE, $initialData );
	}
	...
	protected function history()
	{
		...
		$replies = new \IPS\Patterns\ActiveRecordIterator(
			\IPS\Db::i()->select( 'nexus_support_replies.*, nexus_support_ratings.*', 'nexus_support_replies',
				...
				NULL,
				NULL,
				\IPS\Db::SELECT_SQL_CALC_FOUND_ROWS + \IPS\Db::SELECT_MULTIDIMENSIONAL_JOINS
			)
		...
		$chart = $chart->render( 'ColumnChart', array(
			'backgroundColor'	=> '#f5f5f5',
			'chartArea'			=> array( 'width' => '97%', 'height' => '81%' ),
			'colors'			=> array( '#394248' ),
			'fontSize'			=> 10,
			'height'			=> 150,
			'vAxis'				=> array( 'gridlines' => array( 'count' => 4 ), 'maxValue' => max( $data ), 'textPosition' => 'none' ),
