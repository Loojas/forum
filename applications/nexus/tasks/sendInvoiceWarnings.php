...
class _sendInvoiceWarnings extends \IPS\Task
{
	...
	public function execute()
	{
		if ( \IPS\Settings::i()->cm_invoice_warning )
		{
			...
			{
				$groupedPurchases[ $purchase->member->member_id ][ $purchase->billing_agreement AND !$purchase->billing_agreement->canceled ? $purchase->billing_agreement->id : 0 ][ $purchase->renewal_currency ][ $purchase->id ] = $purchase;
			}
			...
			foreach ( $groupedPurchases as $memberId => $_groupedPurchases )
			{
				...
				foreach ( $_groupedPurchases as $billingAgreementId => $__groupedPurchases )
				{
					foreach ( $__groupedPurchases as $currency => $purchases )
					{
						...
						if ( $billingAgreement )					
						{
							$email = \IPS\Email::buildFromTemplate( 'nexus', 'invoiceWarning', array( array(), NULL, $billingAgreement, $invoice, $invoice->summary() ), \IPS\Email::TYPE_TRANSACTIONAL );
						}
