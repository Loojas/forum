...
class _Package extends \IPS\nexus\Invoice\Item\Purchase
{
	...
	public static function createFromForm( array $values, \IPS\nexus\Invoice $invoice )
	{
		...
		foreach ( array_filter( $data ) as $id => $qty )
		{
			...
			for ( $i = 0; $i < $qty; $i++ )
			{
				...
				/* Base price */
				$basePrice = $package->price( $invoice->member, TRUE, TRUE, TRUE, $initialCount + $i );
				...
				if ( $package->stock == -2 )
				{
					try
					{
						$chosenOption = \IPS\Db::i()->select( '*', 'nexus_product_options', array( 'opt_package=? AND opt_values=?', $package->id, json_encode( $details ) ) )->first();
	...
	public static function acpSupportView( \IPS\nexus\Purchase $purchase )
	{
		try
		{
			return call_user_func_array( array( \IPS\nexus\Package::load( $purchase->item_id ), 'acpSupportView' ), func_get_args() );
		}
		catch ( \OutOfRangeException $e )
		{
			return '';
	...
	public static function onCancelWarning( \IPS\nexus\Purchase $purchase )
	{
		try
		{
			return call_user_func_array( array( \IPS\nexus\Package::load( $purchase->item_id ), 'onCancelWarning' ), func_get_args() );
		}
		catch ( \OutOfRangeException $e )
		{
			return NULL;
